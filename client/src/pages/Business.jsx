...
export default function Business() {
  ...
  // Only color theme changed to cyan shades
  ...
  const highlightStyle = {
    weight: 3,
    color: "#006064", // dark cyan highlight
    fillOpacity: 0.9,
  };
...
<div className="bg-cyan-600 text-white px-4 py-2 flex justify-between items-center">
  {/* Left side: Icon + Title */}
  <div className="flex items-center space-x-2">
    <Briefcase className="w-5 h-5" />
    <h3 className="font-semibold text-sm sm:text-base">Business Units</h3>
  </div>
  ...
  <div className="flex items-center space-x-2 z-[1001]">
    ...
    <button
      ...
      className="px-2 py-1 bg-white text-cyan-700 rounded shadow relative z-[1001] flex items-center justify-center"
    >
      <MagnifyingGlassIcon className="w-4 h-4" />
        ...
        <div className="relative h-[800px]">
          ...
          <div className="absolute top-4 right-4 bg-white/30 dark:bg-white-900/30 backdrop-blur-md shadow rounded-lg p-2 text-xs z-[1000] w-40">
            <h3 className="font-semibold mb-2 text-gray-700 dark:text-gray-700 text-center">Businesses</h3>

            <ul className="flex flex-col items-center gap-1">
              {[
                { color: "#fff2e6", label: "0 - 400" },
                { color: "#ffe0b3", label: "400 - 800" },
                { color: "#ffcc99", label: "800 - 1.2k" },
                { color: "#cc9966", label: "1.2k - 1.6k" },
                { color: "#996633", label: "1.6k - 2.0k" },
                { color: "#663300", label: "2.0k+" },
              ].map((item, idx) => (
                <li key={idx} className="flex items-center gap-2 w-full max-w-[120px]">
                  <span
                    className="w-3 h-3 block flex-shrink-0"
                    style={{ background: getColor(parseInt(item.label)), opacity: 0.7 }}
                  ></span>
